...
public class Money
{
   ...
   {
		boolean equal = true;
		if(amount.dollars != this.dollars){
			equal = false;     //if dollars is not equal to this.dollars, it is set to false and that is returned
		}
	...
	{
		String doll = Long.toString(dollars);
		String cen = "";
		if(cents < 10){
			cen = "0" + Long.toString(cents);
		}
		if(cents > 10){
			cen = Long.toString(cents);
		}
		String amount = doll + "." + cen;
		return amount;
   }
}